...
def writeCheck(db_conn, sanity, timeout):
    ...
    sql_string = ''.join(['INSERT INTO data(time_needed, ping_average, time_start, dbm, ssid_fk, bssid_fk) VALUES(', str(time_needed), ', ', str(ping_average), ', ', str(int(sanity['time_start'])), ', ', str(sanity['dbm']), ', ', str(ssid_id), ', ', str(bssid_id) + ')'])
