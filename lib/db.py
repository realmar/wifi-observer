...
def writeCheck(db_conn, sanity, timeouts):
    ...
    sanity['time_needed_conn'] = float("{0:.2f}".format(sanity['time_needed_conn']))
    sanity['time_needed_dhcp'] = float("{0:.2f}".format(sanity['time_needed_dhcp']))

    time_needed_conn = 'NULL' if sanity['time_needed_conn'] > timeouts['conn'] else str(sanity['time_needed_conn'])
    time_needed_dhcp = 'NULL' if sanity['time_needed_dhcp'] > timeouts['dhcp'] else str(sanity['time_needed_dhcp'])
